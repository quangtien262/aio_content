...
$sourceFile = $argv[1] ?? $root.'/content/programming-articles.php';
if (! str_starts_with($sourceFile, $root)) {
    $sourceFile = $root.'/'.ltrim(str_replace('\\', '/', $sourceFile), '/');
}
$articles = require $sourceFile;
...
foreach ($articles as $filename => $locales) {
    foreach ($locales as $locale => $article) {
        ...
        if ($brand instanceof DOMElement) {
            $brand->textContent = $locale === 'en'
                ? ($article['category_en'] ?? 'Programming')
                : ($article['category_vi'] ?? 'Lập trình');
        }
...
function replaceProgrammingArticleHtml(DOMDocument $target, DOMElement $element, string $html): void
{
